...
public class Application {
    public static void main(String[] args){
        ...
        /*
        * 객체 지향 프로그래밍이란?
        * 현실 세계에 모든 사건(event) 는 객체와 객체의 상호작용에 의해 일어난다는 세계관을
        * 프로그램을 만들때 이용하여 새로운 세계를 창조하는 방법론이다.
        * 현실 세계와 닮아있지만, 다른 점은 객체간의 상호작용을 한다는 사실은 현실 세계와 동일하지만
        * 의인화 기법이 적용되어 현실세계에서 불가능한 무생물이나 개념 같은 존재들도
        * 하나의 주체로 본인의 상태를 스스로 제어하고 행동하도록 한다는 것이 다른점이다.
        ...
        * 상호작용에 중요한 것은 메세지를 송수신하고 그 방법을 기술하여 메세지에
        * 응답하는 상호작용을 설계하는 것이다.
        * 따라서 객체를 설계할 때는 속성은 부가적이며 적절한 책임을 가진 객체들 간의
        ...
        * 4. 카레이서는 먼저 자동차에 시동을 건다. 이미 걸려있는 경우에는 다시 걸 수 없다.
        * 5. 카레이서가 엑셀을 밟으면 시동이 걸린 상태일 경우 10km/h씩 증가하며 앞으로 나간다.
